...
print("all equal to 16:", all(len(v) == 16 for v in std_electrode_names_fixedlen.values()))
...
standard_edf_10_10_electrode_labels_strip = {
    k: v.strip() for k,
    v in iteritems(std_electrode_names_fixedlen)}
label_strip2electrodes = {
    v: k for k,
    v in iteritems(standard_edf_10_10_electrode_labels_strip) if k != 'T1' and k != 'T2'}
label2electrodes = {v: k for k, v in iteritems(std_electrode_names_fixedlen) if k != 'T1' and k != 'T2'}
...
standard_annotations = [
    ...
    'Hypopnea', 'Obstructive hypopnea', 'Central hypopnea',
...
ecg_labels = {k: "ECG %s" % k for k in ecg_leads}  # these are not expanded to 16 characters
